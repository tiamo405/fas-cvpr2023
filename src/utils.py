...
root = os.getcwd()
pwd = os.path.dirname(os.path.realpath("models"))
...
def save_txt_array(path_save, fnames, scores, remove = False) :
    ...
    for i in range(len(fnames)) :
        write_txt(fnames[i]+ ' ' + str(scores[i]), path_save)

def save_zip(folder_save, remove = True) :
    path_save = os.path.join(folder_save, 'submit.zip')
    file_path = os.path.join(folder_save, 'submit.txt')
    if os.path.exists(path_save) and remove == True:
        os.remove(path_save)
...
if __name__ =="__main__" :
    scores = []
    fnames = []
    path_save = "results/dev/submit.txt"
    with open("data/txt/dev.txt", 'r') as f :
        for line in f :
            fnames.append('dev/'+line.split()[0])
            # scores.append(random.randint(0, 10**4)/ 10**4)
            scores.append(random.choice([0.0, 0.6]))
            # write_txt(fnames[-1]+ ' ' + str(scores[-1]), path_save, remove= True)
    save_txt_array(path_save, fnames, scores, remove= True)
